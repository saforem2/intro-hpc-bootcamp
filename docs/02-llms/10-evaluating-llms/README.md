...
- [🤔 Why Evaluation Is Hard](#thinking-why-evaluation-is-hard)
- [🗂️ Categories of
  Evaluation](#card_index_dividers-categories-of-evaluation)
  - [(a) Automatic benchmarks](#a-automatic-benchmarks)
  - [(b) LLM-as-a-judge](#b-llm-as-a-judge)
  - [(c) Human evaluation & Chatbot
    Arena](#c-human-evaluation--chatbot-arena)
  - [(d) Verifiable / programmatic
    evaluation](#d-verifiable--programmatic-evaluation)
- [📏 Metrics That Matter](#straight_ruler-metrics-that-matter)
  - [Perplexity](#perplexity)
  - [Accuracy and `pass@k`](#accuracy-and-passk)
  - [Win-rate](#win-rate)
- [👋 Hands On: A Tiny Eval Harness](#wave-hands-on-a-tiny-eval-harness)
  - [A real harness (display-only)](#a-real-harness-display-only)
...
[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/saforem2/intro-hpc-bootcamp-2025/blob/main/docs/02-llms/10-evaluating-llms/index.ipynb)
[![](https://img.shields.io/badge/-View%20on%20GitHub-333333?style=flat&logo=github&labelColor=gray.png)](https://github.com/saforem2/intro-hpc-bootcamp-2025/blob/main/content/02-llms/10-evaluating-llms/index.qmd)

By now you have trained a model ([\[06\] Parallel
...
learning ([\[15\] RL & Reasoning](../15-rl-and-reasoning/index.qmd)).
This capstone ties all of that together with one deceptively simple
question:
...
Figure 1: Four families of LLM evaluation, from cheap-and-automatic
(benchmarks) to expensive-and-human (arena).
...
> (RLVR)** from [\[15\] RL &
> Reasoning](../15-rl-and-reasoning/index.qmd). There, a programmatic
> verifier (test suite, math checker) *is* the reward signal used to
> train the model. Here, that same verifier *is* the evaluation metric.
> Train-time reward and eval-time metric are two faces of the same
> function — which, foreshadowing, is precisely why [reward
> hacking](#sec-pitfalls) is so dangerous.

## 📏 Metrics That Matter

### Perplexity

Recall from [\[00\] Intro to LLMs](../00-intro-to-llms/index.qmd) that
...
>
> This is the deep connection to [\[15\] RL &
> Reasoning](../15-rl-and-reasoning/index.qmd). When a metric becomes a
> *training target* (RLVR reward, or “optimize for the benchmark”), the
> model learns to maximize the *measure* rather than the *thing you
> meant*. A code model may pass tests by hard-coding expected outputs; a
> chat model rewarded for length may become needlessly verbose.
>
...
`EleutherAI/pythia-160m`, or the mini-LLM you built in [\[00\] Intro to
LLMs](../00-intro-to-llms/index.qmd)) and a small task (a slice of
