...
colored cells show how much it attends to each other token. Use the dropdown to
switch layers — notice how, in several heads/layers, **"it" attends strongly back
to "animal"**:

```{python}
#| label: fig-attention-viz
#| fig-cap: "Real BERT self-attention for the sentence — each row shows what a token attends to. Switch layers with the dropdown."
#| code-fold: true
...
# average over heads for a clean per-layer view
per_layer = [a[0].mean(0).numpy() for a in _att]   # each [seq, seq]

fig = go.Figure()
for li, M in enumerate(per_layer):
    fig.add_heatmap(z=M[::-1], x=_toks, y=_toks[::-1], colorscale="Blues",
                    visible=(li == 5), showscale=True,
                    hovertemplate="%{y} → %{x}: %{z:.2f}<extra></extra>")
fig.update_layout(
    height=520, width=560, title="Attention (mean over heads)",
    xaxis=dict(title="attended-to token", tickangle=-45),
    yaxis=dict(title="attending token"),
    updatemenus=[dict(active=5, x=1.0, y=1.18, xanchor="right",
        buttons=[dict(label=f"layer {i}", method="update",
                      args=[{"visible": [j == i for j in range(len(per_layer))]}])
...
a DistilBERT model fine-tuned for sentiment (SST-2). Heavy download, so
display-only here:
