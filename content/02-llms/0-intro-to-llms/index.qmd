...
> Both are highly recommended companion reading.
...
inputs. This makes them ideal for short sequences in NLP and time-series
prediction.
...
- **Slow to train.** Each step depends on the previous hidden state, so
  computation is inherently **sequential** and can't be parallelized across the
  sequence.
...
# nbformat is what lets plotly render a figure inside a notebook; without it
# every fig.show() below raises "Mime type rendering requires nbformat>=4.2.0".
%pip install transformers torch pandas rich \
    umap-learn plotly scikit-learn nltk bertviz nbformat
```

```{python}
# This page uses the `bootcamp` helper package (the shared plotly theme). It
# lives in this repo, so it is NOT on PyPI and is missing from Colab and from
# site-provided HPC kernels alike -- hence `ModuleNotFoundError: No module
# named 'bootcamp'`. Install it if absent; locally this is a no-op.
try:
...
::: {.callout-warning title="🧩 On a preinstalled HPC kernel, add the missing bits"}
Site-provided kernels (e.g. NERSC's `pytorch-2.11.0` module) ship torch and
`transformers` but not the notebook-only extras, so the plotly cells fail with:

```
ValueError: Mime type rendering requires nbformat>=4.2.0 but it is not installed
```

Nothing is wrong with the figure code — plotly needs `nbformat` to emit the
figure as a notebook MIME bundle. Install into your user site-packages, then
restart the kernel:

```bash
python3 -m pip install --user nbformat nltk bertviz
```

If you would rather not install anything, `fig.write_html("plot.html")` saves
the figure to a file you can open from the Jupyter file browser instead.

The same applies to **`bootcamp`** (`No module named 'bootcamp'`): it is this
repo's own helper package, not a PyPI release, so it is absent from every
environment except a local `pip install -e .`. The cell above installs it
straight from GitHub:

```bash
python3 -m pip install --user "git+https://github.com/saforem2/intro-hpc-bootcamp"
```

It only supplies the shared plotly styling, so if you are blocked from
installing, deleting the `from bootcamp.plotly_theme import apply_theme` and
`apply_theme()` lines leaves the figures intact, just unthemed.
:::
...
::: {.callout-warning title="🔒 On NERSC: point the HF cache at `$SCRATCH` first"}
`transformers` takes a **file lock** before it downloads a model, so two
processes can't corrupt a shared cache. The default cache lives in `$HOME`,
which on Perlmutter is **GPFS — a filesystem that does not implement
`flock()`**. The lock call fails before the download even starts:

```
OSError: [Errno 524] Unknown error 524
```

Errno 524 is `ENOTSUPP` ("operation not supported"), and the traceback bottoms
out in `fcntl.flock(...)` inside `huggingface_hub`. Nothing is wrong with your
code. Point the cache at a filesystem that *does* support locking:
...
**This must run before anything imports `huggingface_hub`.** That library reads
`HF_HOME` *once*, at import time, and freezes the result in a module constant —
so setting the variable in a later cell has no effect, and you get the identical
Errno 524. If you already hit the error in this kernel, the import has happened:
**`Kernel → Restart Kernel`**, then run the cell above *first*.

On ALCF the same trick applies with `/lus/eagle/...` (Polaris) or
`/lus/flare/...` (Aurora). Setting `HF_HOME` in your shell before launching
Jupyter avoids the ordering problem entirely, and `HF_HUB_OFFLINE=1` skips the
lock path when the model is already cached.
:::
...
That's the whole black box: **text in, text out**. (`do_sample=True` and friends
are *decoding* settings, i.e. how the model picks each next token; we dig into
temperature and top-p in [\[2.5\] Decoding & Sampling](../5-decoding-and-sampling/index.qmd).)
The rest of this session is about what happens *inside*.
...
# use_cache=False drops `past_key_values` from the output. Ask for `.attentions`
# by NAME rather than `outputs[-1]`: the field order of a ModelOutput is not
# stable across transformers releases, and on newer versions the last entry is
# the KV cache, which makes bertviz die with "IndexError: tuple index out of
# range" deep inside num_heads().
outputs = model(inputs, use_cache=False)
attention = outputs.attentions          # tuple: n_layers x [1, heads, seq, seq]
...
# Scale the run to the hardware. On a GPU this trains the model the section
# above actually describes (batch 16, 5000 steps); on a CPU it falls back to a
# smaller budget so a laptop build still finishes in about a minute. The
# published page was rendered on a GPU, so the loss curve and the sample below
# are the full-size run.
if device == "cuda":
    batch_size, max_iters, eval_interval = 16, 5000, 500
else:
    batch_size, max_iters, eval_interval = 4, 500, 50
...
print(f"training on {device}: batch_size={batch_size}, max_iters={max_iters}")
```
...
### Train it, and watch it learn

An **untrained** model just emits noise. Let's sample from it *before* training so
we have a baseline, then run the training loop and sample again:

```{python}
model = LanguageModel().to(device)
n_params = sum(p.numel() for p in model.parameters())
print(f"model has {n_params/1e3:.1f}K parameters, training on {device}")

# a fresh, untrained model: pure gibberish
...
The loss falls from ~4.2 (random over the vocabulary, $\ln$(vocab size)) as the
model learns character statistics: to roughly 2.5 in the short CPU run, and down
toward ~1.5 in the full GPU run above. That difference is worth noticing. It is
the same architecture and the same code; only the compute budget changed, and
the extra steps are what turn noise into something readable. Now sample again,
using the **same** `generate()` call but from the *trained* weights:

```{python}
print("--- AFTER training ---")
print(decode(model.generate(ctx, max_new_tokens=400)[0].tolist()))
```

It won't write *Hamlet*; it's a ~200K-parameter character model and a few
minutes of training. But the transformation is unmistakable: the gibberish becomes
line-broken text with capitalized speaker names, plausible word shapes, and
Shakespearean punctuation. That is a language model, built from the pieces above,
learning purely by predicting the next character. Train longer / bigger (as in
[\[2.3\] Shakespeare from Scratch](../3-shakespeare-ezpz/index.qmd)) and it keeps
getting better.
...
directory (shown, not run here, since it would write a model folder to disk):
...
Encoder-only models use **bidirectional** attention, so every token can see the
...
only to earlier positions. That is essential for learning generation without
...
- **Attention replaced recurrence**, enabling parallel training and long-range
  context. This was the breakthrough behind LLMs.
