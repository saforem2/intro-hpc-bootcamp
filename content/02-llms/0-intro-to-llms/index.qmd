...
The loss falls from ~4.2 (random over the vocabulary, $\ln$(vocab size)) toward
~2 as the model learns character statistics. Now sample again, using the **same**
`generate()` call but from the *trained* weights:
...
It won't write *Hamlet*; it's a ~200K-parameter character model trained for a few
hundred steps. But the transformation is unmistakable: the gibberish becomes
