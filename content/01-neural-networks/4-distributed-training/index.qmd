...
![**DDP vs. FSDP.** In DDP every rank holds a *full copy* of the model, gradients,
and optimizer states — simple, but memory scales with model size. FSDP (ZeRO-3)
**shards** all three across ranks and reconstructs each layer's params on the fly
with `all-gather`, trading extra communication for a large memory saving.](./assets/ddp-vs-fsdp.svg){#fig-ddp-vs-fsdp .column-body-outset}
