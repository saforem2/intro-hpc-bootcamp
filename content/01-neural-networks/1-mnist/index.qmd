...
**Prerequisites:** the [0] primers on [Python](../../00-intro-AI-HPC/3-python/index.qmd) and
...
in a few minutes on CPU, with no GPU required (a GPU only speeds up the training
loop, it changes nothing about the results).

**What success looks like:** the simple **linear** model should reach roughly
**~91%** test accuracy, and the deeper **nonlinear** model should climb a few
points higher into the **~93–94%** range. If your numbers are in that ballpark,
it's working. (Training longer, or with a larger network, pushes the nonlinear
model into the mid-to-high 90s, a good thing to try for the homework.)
:::
...
Let's begin with a simple linear model: linear regression, as in [\[0.6\]](../../00-intro-AI-HPC/6-linear-regression/). 
...
- A loss function $J(\theta)$ where $\theta$ is the list of parameters (here W
  and b).
  Earlier we used mean squared error (MSE), but here let's make two
...
- An optimization method or optimizer such as the stochastic gradient descent
  ...
  stochastic gradient descent (sgd), as before. For far more information
...
- A learning rate.
  As we saw earlier, the learning rate controls how far we move during
  each step.
...
panel) is lowest for the balanced model, and that's the sweet spot we want.
...
the training set, and training keeps improving while **validation turns the
wrong way**. The gap between the two curves *is* overfitting, and the dashed line
marks where you'd want to **stop early**:
...
getting worse on data it *hasn't*. That is the definition of overfitting.
...
<!--  <img src="images/test_data_rule.png" width="800" height="500" align="center"/>
  -->
...
Overall accuracy is a single number; it hides *where* the mistakes are. A
**confusion matrix** answers "when the true digit is a 4, what does the model
guess?" Row $i$, column $j$ counts test examples whose true label is $i$ but
that the model predicted as $j$. A perfect model is all diagonal; the bright
*off*-diagonal cells are the pairs it mixes up (classically 4↔9, 3↔5, 7↔9).

```{python}
import numpy as np

# Run the trained model over the whole test set and collect (true, predicted).
y_true, y_pred = [], []
nonlinear_model.eval()
with torch.no_grad():
    for X, y in test_dataloader:
        logits = nonlinear_model(X)
        y_pred.append(logits.argmax(1))
        y_true.append(y)
y_true = torch.cat(y_true).numpy()
y_pred = torch.cat(y_pred).numpy()

# 10x10 counts. cm[i, j] = # of true-i examples predicted as j.
cm = np.zeros((10, 10), dtype=int)
np.add.at(cm, (y_true, y_pred), 1)

# Normalize each row to a *rate* so classes with more examples don't dominate.
cm_rate = cm / cm.sum(axis=1, keepdims=True)
```

```{python}
fig = go.Figure(go.Heatmap(
    z=cm_rate, x=list(range(10)), y=list(range(10)),
    colorscale="Blues", zmin=0, zmax=1,
    hovertemplate="true=%{y}, pred=%{x}<br>rate=%{z:.2%}<extra></extra>",
))
fig.update_layout(
    xaxis_title="predicted digit", yaxis_title="true digit",
    yaxis=dict(autorange="reversed"), height=460,
    title="Confusion matrix (row-normalized)")
fig.show()

# Call out the single worst off-diagonal confusion.
off = cm_rate.copy()
np.fill_diagonal(off, 0.0)
i, j = np.unravel_index(off.argmax(), off.shape)
print(f"most-confused pair: true {i} -> predicted {j} "
      f"({off[i, j]:.1%} of all {i}s)")
```

The diagonal should be overwhelmingly the brightest band, since the model is
right most of the time, and the worst confusion is usually a genuinely ambiguous
pair a human would also hesitate on. This is the kind of per-class diagnostic
you'd use to decide *what to fix next* (more data for a weak class, a better
architecture, targeted augmentation).
