...
It did ... OK.
Note that our demo run above is deliberately short — only `TRAIN_ITERS = 100`
steps at batch size 16 (~1600 samples), which is far less than a single pass over
the ~48k training images. With such a short run you won't see overfitting yet;
the model is still underfitting.
As an exercise, bump `TRAIN_ITERS` up (or train for several full epochs using the
`train_one_epoch` loop below) and watch the training and validation curves start
to diverge — that gap is overfitting, and there are ways to address it (data
augmentation, weight decay, dropout) that we didn't have time to get into in this
session.
