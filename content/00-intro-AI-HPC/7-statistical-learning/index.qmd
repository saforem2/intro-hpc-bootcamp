...
```{python}
# Same `x_sc` (1000 points, 5 blobs) we generated and standardized at the top of
# the page -- no new data here, so what you watched in the scatter above is
# exactly what the algorithm is clustering now.
n_clusters_guess = n_clusters   # 5: what we'd guess if we could see the plot
iterations = 50
...
def kmeans_frame(cents, cid):
    # ONE trace holding every point, always in the same order, with only the
    # per-point COLOR changing between frames. Splitting the points into one
    # trace per cluster (the obvious approach) makes each trace change length
    # as points are reassigned -- and because plotly animates trace i to trace
    # i by interpolating x/y elementwise, the points appear to *slide across
    # the plot* instead of simply recoloring. The data never moves; only the
    # centroids (★) should.
    colors = [_cats[int(c) % len(_cats)] for c in cid]
    return [
        go.Scatter(x=x_sc[:, 0], y=x_sc[:, 1], mode="markers",
                   marker=dict(size=5, opacity=0.5, color=colors)),
        go.Scatter(x=cents[:, 0], y=cents[:, 1], mode="markers",
...
### 🎛️ How many clusters should you ask for?

k-means never *discovers* the number of clusters — you tell it `k`, and it
dutifully returns exactly that many, whether or not they mean anything. Use the
buttons to force the same 1000 points into 2, 3, 4, 5, 6 or 8 clusters and
watch what it does with a `k` the data does not support:

```{python}
#| code-fold: true
#| code-summary: "Show the plotting code"
k_choices = [2, 3, 4, 5, 6, 8]

# Re-fit for each k and record how tight the clusters are. Inertia = the total
# squared distance from every point to its own centroid: lower is "tighter",
# but it ALWAYS falls as k grows (with k = n_samples it would hit zero), which
# is exactly why you cannot just minimize it.
def inertia_of(c, cid):
    """Total squared distance from every point to its own centroid."""
    return float(((x_sc - c[cid]) ** 2).sum())

fits, inertias = {}, {}
for k in k_choices:
    # Best of a few restarts. A single run can land in a bad local minimum --
    # at k=5 one unlucky seed gives inertia 103 instead of 64, which would put
    # a fake kink in the curve. Real implementations (sklearn's `n_init`) do
    # exactly this.
    best = min(
        (apply_kmeans(x_sc, k, iterations, seed=sd, cmap=cmap, init="kmeans++")[:2]
         for sd in range(5)),
        key=lambda cc: inertia_of(*cc),
    )
    fits[k] = best
    inertias[k] = inertia_of(*best)

def k_traces(k):
    c_k, cid_k = fits[k]
    return kmeans_frame(c_k, cid_k)   # same single-trace-of-points helper as above

fig_k = go.Figure(data=k_traces(5))
fig_k.update_layout(
    height=460, showlegend=False,
    yaxis=dict(scaleanchor="x", scaleratio=1),
    title=f"k = 5 &nbsp;·&nbsp; inertia = {inertias[5]:.0f}",
    updatemenus=[dict(
        type="buttons", direction="right", showactive=True, active=3,
        x=0.0, y=1.14, xanchor="left",
        buttons=[dict(label=f"k = {k}", method="update",
                      args=[{"x": [t.x for t in k_traces(k)],
                             "y": [t.y for t in k_traces(k)],
                             "marker": [t.marker for t in k_traces(k)]},
                            {"title": f"k = {k} &nbsp;·&nbsp; inertia = {inertias[k]:.0f}"}])
                 for k in k_choices])],
)
fig_k.show()
```
...
- **The data has five blobs, but every `k` "works".** At `k = 2` and `k = 3`
  whole blobs get lumped together; at `k = 6` and `k = 8` real blobs are split
  down the middle by boundaries that mean nothing. The algorithm never
  complains — it cannot tell you that you asked for the wrong number.
- **Inertia always improves**, so you cannot just minimize it. Watch the number
  in the title as you step up: it keeps falling hard while each new cluster is
  still finding a *real* blob (`k = 2 → 3 → 4 → 5`), then flattens out once
  there are no blobs left to find. That bend at `k = 5` is the signal, and
  plotting inertia against `k` to look for it is the **elbow method** — the same
  plot we use on the cancer data below, where the bend is far less obvious.
