...
dropdown to force the same 500 points into 2, 3, 4, 5, or 6 clusters and watch
what it does with a `k` the data does not support:
...
fits, inertias = {}, {}
for k in k_choices:
    c_k, cid_k, _ = apply_kmeans(x_sc, k, iterations, seed=0, cmap=cmap, init="kmeans++")
    fits[k] = (c_k, cid_k)
    inertias[k] = float(sum(((x_sc - c_k[cid_k]) ** 2).sum(axis=1)))

def k_traces(k):
    c_k, cid_k = fits[k]
    return kmeans_frame(c_k, cid_k)   # same single-trace-of-points helper as above

fig_k = go.Figure(data=k_traces(3))
fig_k.update_layout(
    height=460, showlegend=False,
    yaxis=dict(scaleanchor="x", scaleratio=1),
    title=f"k = 3 &nbsp;·&nbsp; inertia = {inertias[3]:.0f}",
    updatemenus=[dict(
...
- **The data has three blobs, but every `k` "works".** At `k = 2` two blobs get
  merged; at `k = 5` a real blob gets split down the middle by a boundary that
  means nothing. The algorithm cannot tell you it made a bad choice.
- **Inertia always improves**, so you cannot just minimize it. Switch between
  the buttons and watch the number in the title: it falls steeply from `k = 2`
  to `k = 3`, then only inches down. That knee is the signal — past `k = 3` you
  are paying for more clusters and getting almost no extra tightness. Plotting
  inertia against `k` and looking for the bend is the **elbow method**, the same
  plot we use on the cancer data below, where the elbow is far less obvious.
